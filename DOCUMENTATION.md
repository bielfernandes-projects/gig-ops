...
* **O que sai:** as políticas antigas por "tenant do admin" (`profiles_select`, `projects_select/write`, `members_select/write`, `gigs_select/write`, `lineup_select/write`), as funções `private.tenant_id()`, `private.is_admin()` e `private.my_gig_ids()`, a coluna `admin_id` de `go_projects`/`go_members`/`go_gigs` (com os triggers `sync_band_admin` e a função), e a tabela `public.go_settings`.
