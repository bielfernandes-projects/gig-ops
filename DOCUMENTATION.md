...
* **Confirm email:** ligado — o e-mail é uma *afirmação de identidade* neste app (o vínculo com `go_members` é por e-mail, em `lib/auth.ts` e na própria RLS, `private.my_gig_ids_v2()`), então sem confirmação alguém que saiba o código de convite e o e-mail de um músico assumiria o lugar dele. O app não assume nenhum dos dois modos: ver §44
...
* **Escolhido na criação da banda.** O onboarding tem um campo `Código de convite (opcional)` na seção do responsável: é o código que essa pessoa vai passar pros músicos. Vazio, o default do banco gera um. A mesma validação (`validateInviteCode` em `lib/bands.ts`) vale aqui e no Perfil (`saveInviteCode`).
...
## 41. Onboarding: "Seu nome" e o código que o responsável cria
* **O problema que isso resolve:** a seção "Sou o responsável pela banda" pedia um **código de indicação** — que é o código de convite de *outra* banda. Quem está criando a própria banda lê aquele campo como "o código que eu vou criar pra chamar meus músicos", e o campo de código de outra banda já estava logo abaixo, na seção "Fui convidado". Num teste real o dono digitou um código novo e levou "código de indicação inválido", que estava tecnicamente certo e completamente sem sentido pra ele.
* **Layout de hoje** (`app/onboarding/page.tsx`):
  * **Seu nome** (opcional, até 40), no topo, fora das duas seções. Como cada seção é um `<form>` próprio e os handlers montam a FormData com `new FormData(e.currentTarget)`, o campo é estado controlado e entra via `fd.set('displayName', ...)` nos dois caminhos (`formDataWithName`).
  * **Sou o responsável:** nome da banda + `Código de convite (opcional)` — o código que essa pessoa passa pros músicos. Vazio, o default do banco gera um.
  * **Fui convidado:** o código de convite que o responsável mandou.
* **Os dois campos de código têm limites diferentes de propósito:** o do responsável é `maxLength={5}` (código inventado, a regra real é 5); o do convidado é `maxLength={12}` porque recebe código de banda que já existe, e bandas migradas de `go_settings` podem ter um código legado mais longo. Em ambos quem decide é o servidor.
...
## 43. `go_profiles.tour_seen_at`: tour lembrado pela conta, não pelo aparelho
* **Migration:** `supabase/migrations/20260926000100_tour_seen_at.sql` — `alter table public.go_profiles add column if not exists tour_seen_at timestamptz;`. aplicada em 2026-09-26. (Enquanto não estava aplicada, o `select` da coluna voltava erro, o app tratava como "não viu" e o tour abria — degradava sem quebrar.)
* **Escrita:** `markTourSeen()` em `app/actions/tour-actions.ts`, chamada quando o tour é concluído ou pulado. Usa `createAdminClient()` porque `go_profiles` não aceita escrita do cliente de sessão (RLS, §297 / `20260921000001_enable_rls.sql`). Erro só vai pro log: o tour já acabou de qualquer forma.
* **Leitura:** `app/dashboard/page.tsx` lê `tour_seen_at` **em paralelo** com a query dos shows (`Promise.all`, sem waterfall) e passa `tourSeen` pro `DashboardClient` → `AppTour`.
* **As duas fontes juntas:** o tour abre quando `?tour=1` está na URL, ou quando o banco **e** o `localStorage` concordam que a pessoa não viu. O `localStorage` (`gg-tour-v1:<userId>:<role>`, §42) continua existindo como guarda local — se a escrita no banco falhar, ele impede o tour de reabrir pra sempre. Sem storage (aba privada, site data bloqueado), a resposta do banco decide sozinha.
* **Resultado:** a mesma conta em outro navegador ou celular não vê o tour de novo, e uma conta nova num navegador veterano vê.

## 44. Confirmação de e-mail: quem decide é o Supabase, não o código
* **Por que a confirmação importa aqui (não é só higiene):** o app usa o e-mail da sessão como afirmação de identidade. `lib/auth.ts` resolve a linha de `go_members` por `email.eq."<email>"`, e a própria RLS faz o mesmo (`private.my_gig_ids_v2()` casa `lower(m.email) = lower(auth.jwt() ->> 'email')`). Sem confirmação, quem soubesse o código de convite da banda e o e-mail de um músico cadastrado criava conta com aquele e-mail e era tratado como ele — cachês e shows inclusive. Pelo Google não há esse risco (o e-mail já vem verificado); o caminho exposto era o cadastro por senha.
* **O app não fixa o modo no código.** `signUp()` devolve sessão quando o autoconfirm está ligado e devolve `session: null` quando a confirmação está ligada. `signup()` e `adminSignup()` (`app/login/actions.ts`) olham `data.session`: com sessão, `enterDirectly()` revalida e manda pro `/dashboard`; sem sessão, devolvem `{ success: true }` e a tela "Quase lá!" aparece. Assim a tela nunca desmente a configuração do projeto — antes o texto "enviamos um link de confirmação" aparecia mesmo com autoconfirm ligado, ou seja, mentia.
* **A criação da banda acontece antes da confirmação e continua funcionando:** `joinBandByCode` e `createBandFor` usam o service role, que não depende de sessão. Quem confirma depois já entra com a banda pronta.
* **Tela "Quase lá!":** ganhou "Voltar para o site" (link pra landing) e "Já confirmei — quero entrar" (volta pro formulário de login). O timer de 10s que trocava a tela sozinha saiu: ele levava embora os botões justamente enquanto a pessoa lia. O link do e-mail cai em `/login` (`emailRedirectTo`), então não há caminho sem saída.

## 45. Limpeza do legado da fase 0
* **Migration:** `supabase/migrations/20260926000200_limpeza_legado_fase0.sql`, aplicada em 2026-09-26.
* **O que sai:** as políticas antigas por "tenant do admin" (`profiles_select`, `projects_select/write`, `members_select/write`, `gigs_select/write`, `lineup_select/write`, e as não versionadas — ver abaixo), as funções `private.tenant_id()`, `private.is_admin()` e `private.my_gig_ids()`, a coluna `admin_id` de `go_projects`/`go_members`/`go_gigs` (com os triggers `sync_band_admin` e a função), e a tabela `public.go_settings`.
* **Por que é seguro:** políticas permissivas se somam com **OR**, então as antigas apenas alargavam o acesso — todo dono de banda passa pelas políticas por banda (`private.is_band_owner(band_id)`) criadas na fase 0. Nenhum código do app lê `admin_id`, `go_settings` ou `invited_by` (conferido). Os dados de `go_settings` viraram colunas de `bands` na fase 0 e seguem em `backup_fase0.go_settings`.
* **`private.is_admin()` era uma armadilha:** ela lia `go_profiles.role` e já retornava `false` pra todo dono de banda criado depois da fase 0. Qualquer política ou código novo que a usasse negaria acesso a quem é dono.
* **Ficou de fora de propósito:** `go_profiles.role` e `go_profiles.invited_by`. A trigger `public.handle_new_user()` foi criada pelo painel, fora das migrations, e pode gravar nessas colunas — se gravar, dropá-las faz todo cadastro novo falhar com "Database error saving new user". Elas saem numa migration seguinte, junto com a versão revisada da função (que aí passa a ser versionada no repo). Pra decidir, rodar no SQL editor: `select pg_get_functiondef('public.handle_new_user()'::regprocedure);`
* **A role de verdade é `band_members.role`** (`owner` | `member`), gravada por `createBandFor`/`joinBandByCode` e lida por `getUserInfo()`. `go_profiles.role` não é lida por nenhum código do app.
* **Políticas criadas pelo painel, fora das migrations:** a primeira tentativa de aplicar a migration falhou com `cannot drop column admin_id of table go_gigs because other objects depend on it / policy reminders_admin on table go_reminders`. Essa `reminders_admin` não existe em migration nenhuma — foi criada direto no painel, como a `handle_new_user()`. Como podem existir outras invisíveis, o passo 1 da migration virou uma varredura (`DO` block em `pg_policies`) que remove toda política que ainda cite `admin_id`, `is_admin()`, `tenant_id()` ou `my_gig_ids()`, restrita às cinco tabelas que já têm política equivalente por banda (`go_projects`, `go_members`, `go_gigs`, `go_lineup`, `go_reminders`). `go_push_subscriptions` fica fora de propósito: a política dela é por usuário e não tem substituta.
  * O filtro usa `my_gig_ids()` **com parênteses** pra não casar com a nova `private.my_gig_ids_v2()` e derrubar as políticas boas.
  * A ordem importa: as políticas saem **antes** das funções, senão o `drop function` falha por dependência.
  * A migration é idempotente (`if exists` em tudo, e a varredura não acha nada na segunda passada), então pode ser rodada de novo sem medo — inclusive depois de uma falha no meio.

## 46. Fim da limpeza: `go_profiles.role`/`invited_by` e a trigger versionada
* **Migration:** `supabase/migrations/20260926000300_drop_go_profiles_role.sql`, aplicada em 2026-09-26. Idempotente.
* **`handle_new_user()` gravava em `role`.** A definição real (obtida com `pg_get_functiondef`) era `INSERT INTO public.go_profiles (id, email, role) VALUES (new.id, new.email, 'viewer')`. Dropar a coluna sem mexer na função faria **todo cadastro novo falhar** com "Database error saving new user" — o mesmo erro da §38. A migration recria a função sem `role` (e com `on conflict (id) do nothing`, defensivo) **antes** de dropar a coluna, e com isso a função passa a ser versionada no repo: ela só existia no painel, que é justamente como esse problema passou perto.
  * `create or replace function` preserva dono e privilégios, então o `REVOKE EXECUTE ... FROM PUBLIC, anon, authenticated` da §297 continua valendo.
* **Correção de uma regressão da migration anterior:** `20260926000200` dropou a política `profiles_select`, que era `id = auth.uid() OR invited_by = auth.uid()` — era **ela** que deixava cada pessoa ler o próprio perfil. A substituta da fase 0, `profiles_band_select`, usa `private.visible_profile_ids()`, que devolve só os membros das bandas onde *quem lê* é dono. Ou seja: um músico que não é dono de banda nenhuma perdeu a leitura da própria linha de `go_profiles` pelo cliente de sessão, quebrando em silêncio o `display_name` no Perfil (`app/profile/page.tsx:14`) e o `tour_seen_at` no Dashboard (`app/dashboard/page.tsx`). A migration cria `profiles_own_select` (`using (id = auth.uid())`); somada por OR com `profiles_band_select`, dá o que o app precisa: cada um lê o seu, e o dono lê os da banda dele.
  * **Lição:** ao trocar uma política por outra, conferir cláusula por cláusula o que a antiga permitia. `profiles_select` tinha dois OR e a substituta só cobria um deles.
* **Cobertura conferida nas outras tabelas:** `go_projects`, `go_members`, `go_gigs`, `go_lineup` e `go_reminders` têm políticas por banda iguais ou mais amplas que as antigas (`is_band_member`/`is_band_owner`), e as leituras que o app faz com o cliente de sessão — `go_members` em `getUserInfo()`, shows, escala — seguem cobertas. `go_reminders` é escrita pelo cliente de sessão do dono (`gig-actions.ts`) e lida pelo cron com service role.

## 47. Painel do produto em `/admin`
Visão de dono de produto: contas, assinaturas, faturamento estimado, gestão de contas e telemetria de uso. Não aparece em navegação nenhuma e está no `disallow` do `app/robots.ts`.

### 47.1. Acesso: duas camadas e três gates
* **Quem pode** — `isSuperAdmin(email)` em `lib/admin.ts`, contra `ADMIN_EMAILS` (lista separada por vírgula). Fica em **env e não no banco** de propósito: o painel apaga contas e bandas, então não pode existir caminho de escalada por dados, nem uma coluna `is_admin` que um bug de escrita marque.
* **Senha de novo na entrada (step-up)** — estar logado no app não abre o painel. `/admin/entrar` pede a senha outra vez; `elevate()` a confere contra o Supabase com um cliente **descartável** (`persistSession: false`, sem cookies), então a sessão de quem navega não é tocada — só se pergunta "essa senha é a dessa conta?". A senha nunca aparece no código: quem guarda é o Supabase, com hash.
* **O cookie de elevação** (`gg_admin`, `lib/admin.ts`) é `httpOnly`, `sameSite: strict`, `path: /admin`, e **sem `maxAge` de propósito** — cookie de sessão, morre quando o navegador fecha. O prazo real (`ELEVATION_MINUTES`, 60) está assinado dentro do token, pra quem nunca fecha o navegador. O token é `<userId>.<expiraEm>.<hmac>` com `ADMIN_SESSION_SECRET`: o `userId` entra na assinatura pra o cookie não servir a outra conta, e a comparação do HMAC é `timingSafeEqual` (um `===` vazaria o prefixo correto byte a byte). Sem o segredo configurado, falha fechado.
* **Os três gates:** (1) `lib/supabase/middleware.ts` devolve **404** — não `redirect`, pra não anunciar que a rota existe — pra quem não está na lista; (2) `app/admin/layout.tsx` refaz a checagem e cada página chama `requireElevatedAdmin()` (`lib/admin-session.ts`); (3) **toda** action em `app/admin/actions.ts` exige as duas camadas de novo. Sem exigir elevação nas actions, a senha de entrada seria decoração — bastaria chamar a action depois de ela expirar. Layout não é segurança: server action é endpoint público.
* A exigência de elevação **não** mora no layout de propósito: a tela de senha vive sob o mesmo layout e entraria em loop consigo mesma. O layout só esconde a navegação e o botão "Sair do painel" enquanto não há elevação.

### 47.2. Números (`lib/admin-stats.ts`)
* `productOverview()` — contas e quantas voltaram em 30 dias, bandas, donos vs. músicos, assinaturas por estado (via `subscriptionState()`, sem reimplementar a regra), MRR e fundadores (`countFounders()` sobre `FOUNDER_LIMIT`). Tudo num `Promise.all` com o cast `as unknown as Promise<...>`, pela regra de waterfall do `AGENTS.md`.
* **MRR:** só assinaturas ativas **com `stripe_subscription_id`** entram, usando `PRICES` de `lib/pricing.ts` (anual rateado por 12, senão o número pula de degrau). Ativa **sem** assinatura no Stripe é banda liberada na mão (amigos/testers, ou Pix) — conta como ativa e fica **fora** do MRR, com número próprio no card, senão o faturamento mente pra cima.
* `listUsers()` — pagina `auth.admin.listUsers` de 200 em 200 (não filtra nem ordena no servidor, mesmo laço de `supabase/scripts/grant-free-access.ts`) e cruza com `go_profiles`, `band_members` e `subscriptions`.

### 47.3. Gestão de contas (`app/admin/actions.ts`)
* `sendPasswordReset(email)` — manda o e-mail de redefinição (não devolve link: link na tela vaza em histórico e em print), com o mesmo `redirectTo` do `forgotPassword`.
* `previewUserDeletion(userId)` → `deleteUser(userId)`. **Apagar nunca é um clique:** a prévia diz exatamente o que morre — as bandas das quais a pessoa é a **única** dona, com contagem de shows, músicos e músicas — e só então o botão libera. O `on delete cascade` de `bands` leva membros, assinatura, shows, escala, projetos, músicas e repertórios. Bandas com outro dono ficam de pé: a pessoa só deixa de ser membro. A action recusa apagar a própria conta do admin.

### 47.4. Telemetria
* **Migration** `supabase/migrations/20260927000000_telemetry.sql`: tabela `app_events` (`kind` 'screen'|'action', `name`, `user_id`, `band_id`, `created_at`), RLS ligada **sem policy nenhuma** e `revoke all from anon, authenticated` — só service role. As FKs são `on delete set null`: apagar um usuário não apaga o histórico de uso.
* **Agregação no banco**, não em JavaScript: `admin_event_counts(p_days)`, `admin_signups_by_day(p_days)` e `admin_user_totals()` são `security definer` (leem `app_events` e `auth.users`). O `revoke execute ... from public` é seguido de `grant ... to service_role` — sem o grant explícito o revoke de `public` tiraria o acesso do service_role também, que é exatamente a armadilha da §46. Nomes de saída com prefixo (`event_kind`, `event_name`) porque iguais aos das colunas o corpo da função fica ambíguo pro Postgres.
* **Telas:** `components/screen-tracker.tsx` (montado em `app/layout.tsx`) manda cada navegação pra `app/api/telemetry/route.ts` com `keepalive`, sem esperar resposta. Ignora landing, login, páginas legais, `/s/`, `/auth/` e `/admin` — nada disso é uso do produto. A rota tira o usuário **da sessão, nunca do corpo** da requisição, senão qualquer pessoa gravaria evento no nome de outra.
* `normalizeScreen()` (`lib/telemetry.ts`) troca uuid e número por `:id`, então `/gigs/<uuid>` vira `/gigs/:id` — sem isso cada show visitado é uma "tela" diferente e a pergunta "quais telas são mais usadas" não tem resposta.
* **Ações-chave:** `logAction()` com nomes num tipo fechado (`ActionName`) — uma string solta trocada por descuido quebraria a agregação sem quebrar nada visível. Instrumentados: `gig_criado`, `cache_pago` (`gig-actions.ts`), `presenca_confirmada`/`presenca_recusada` (`presence-actions.ts`), `musico_cadastrado` (`member-actions.ts`), `musica_criada` (`song-actions.ts`), `repertorio_criado` (`setlist-actions.ts`), `banda_criada`/`entrou_na_banda` (`onboarding/actions.ts`).
* **Telemetria nunca derruba a ação do usuário:** `logEvent` engole o erro no log. Ninguém perde um show cadastrado porque o registro falhou.
* **Retenção:** o cron que já existe (`app/api/cron/check-reminders`) apaga eventos com mais de 90 dias. Sem infra nova.

### 47.5. Gráficos
Série única em todos (magnitude), então **hue único e sem legenda** — o título nomeia a série. A cor é `#059669` (emerald-600), escolhida pelo validador da skill `dataviz` e não a olho: emerald-500 (`#10b981`, o accent do app) reprova na faixa de luminosidade do modo escuro (L 0,696 contra a faixa 0,48–0,67), e o 600 passa nos seis checks contra a superfície real `#18181b`.
