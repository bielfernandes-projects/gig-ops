...
export async function saveInviteCode(formData: FormData) {
  ...
  const checked = await validateInviteCode(String(formData.get('inviteCode') ?? ''), ctx.bandId);
  if ('error' in checked) return { error: checked.error };

  const { error } = await ctx.supabase.from('bands').update({ invite_code: checked.code }).eq('id', ctx.bandId);
  if (error) {
