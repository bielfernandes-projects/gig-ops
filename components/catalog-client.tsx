...
function letrasSearch(title: string, artist: string) {
  return `https://www.letras.mus.br/?q=${encodeURIComponent(`${title} ${artist}`.trim())}`;
}
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 text-xs font-medium text-zinc-400">
          Tom que começa (pedido à harmonia)
      ...
      <label className="flex flex-col gap-1 text-xs font-medium text-zinc-400">
        <span className="flex items-center justify-between gap-3">
          Link da letra (opcional)
          <a
            href={letrasSearch(title, artist)}
            target="_blank"
            rel="noopener noreferrer"
            className="font-normal text-zinc-500 underline underline-offset-4 hover:text-zinc-300"
          >
        ...
        {filtered.map((s) => {
          ...
          return (
            <li key={s.id} className="flex items-center justify-between gap-3 px-4 py-3">
              <button type="button" onClick={() => setViewing(s)} className="min-w-0 flex-1 text-left" title="Ver detalhes">
                ...
                <p className="flex flex-wrap items-center gap-x-1.5 text-xs text-zinc-500">
                  ...
                  {s.start_key && <span className="rounded bg-amber-300 px-1.5 py-0.5 font-bold text-black">Tom que começa {s.start_key}</span>}
                  <span>{[s.bpm && `${s.bpm} BPM`, !s.source_url && !s.pdf_path && !s.chart_text && 'sem cifra'].filter(Boolean).join(' · ')}</span>
                </p>
              </button>
              <div className="flex shrink-0 items-center">
                {s.source_url && (
                  <a href={s.source_url} target="_blank" rel="noopener noreferrer" title="Abrir cifra" className="p-2 text-zinc-500 hover:text-zinc-200">
                    <FileText className="h-4 w-4" />
                  </a>
                )}
                {s.lyrics_url && (
                  <a href={s.lyrics_url} target="_blank" rel="noopener noreferrer" title="Abrir letra" className="p-2 text-zinc-500 hover:text-zinc-200">
